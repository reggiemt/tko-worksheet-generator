...
When a problem benefits from a diagram, set hasVisual=true and provide a detailed figureDescription in plain English. The figureDescription must specify:
- The type of figure (parallel lines with transversal, triangle, coordinate plane, etc.)
- All geometric elements (lines, points, angles, sides)
- Exact labels and their positions (e.g., "Label vertex A at bottom-left, B at bottom-right, C at top")
- Side length labels and where they go (e.g., "Label side AB = 8, centered below the base, outside the triangle")
- Angle labels and which specific angle region they mark (e.g., "Label the acute angle below-right of the upper intersection as x°")
- Any special markings (right angle squares, parallel arrows, tick marks for equal sides)

Be VERY specific about label positions — say "outside the triangle on the left side" not just "label it 8".
A separate system will generate the actual TikZ code from your description.
  ...
  "problems": [
    {
      ...
      "isGridIn": false,
      "hasVisual": true,
      "figureDescription": "Two horizontal parallel lines labeled p (top) and q (bottom), cut by a diagonal transversal going from upper-left to lower-right. The transversal extends beyond both lines. At the upper intersection with line p, label the acute angle on the lower-right side (between line p going right and the transversal going down-right) as x°. At the lower intersection with line q, label the acute angle on the upper-left side (between the transversal going up-left and line q going left) as y°. These are alternate interior angles."
    },
    {
      "number": 2,
...
2. All LaTeX backslashes in content fields must be escaped as \\\\ in JSON strings
...
6. figureDescription should be null (not an empty string) when hasVisual is false
7. NEVER write "in the figure below" or "shown below" in problem text UNLESS you are also setting hasVisual=true AND providing a detailed figureDescription. If you cannot describe a figure, rewrite the problem to be self-contained.
8. figureDescription must be EXTREMELY detailed — specify every label, every position, every side length placement. A TikZ artist who has never seen the problem must be able to draw it perfectly from your description alone.`;

export const TIKZ_SYSTEM_PROMPT = `You generate TikZ and pgfplots figures for SAT-style math worksheets. You will receive a problem description and a figure description. Output ONLY the TikZ code — a complete \\begin{tikzpicture}...\\end{tikzpicture} environment. No explanations, no JSON, no markdown code blocks.
