...
export function AccountStatus() {
  ...
  const tierIcon = usage.tier === "pro" ? Crown : usage.tier === "starter" ? Zap : User;
  const TierIcon = tierIcon;
  const tierLabel = usage.tier === "free" ? "Free" : usage.tier === "starter" ? "Starter" : "Pro";
  const tierColor = usage.tier === "pro" ? "text-purple-600" : usage.tier === "starter" ? "text-blue-600" : "text-slate-500";
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-slate-600">
            {usage.remaining} of {usage.limit} worksheets left {periodLabel}
          </span>
        </div>
        {expanded ? (
